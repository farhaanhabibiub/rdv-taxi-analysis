...
for tabel, query in tables.items():
    ...
    result.append({'tabel': tabel, 'jumlah_baris': f'{jumlah:,}'})
...
print(con.execute('SELECT trip_id, operator_code, PULocationID, DOLocationID, pickup_datetime, trip_duration_min, trip_distance, fare_amount, tip_amount, total_amount FROM fact_trips LIMIT 5').df().to_string(index=False))
